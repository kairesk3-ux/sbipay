  ...
  if (request.method === 'POST' && pathname === '/api/register') {
    try {
      ...
      if (shouldUseMongo()) {
        ...
        await users.insertOne({username, phone, passwordHash: await hashPassword(password), userId, inviteCode, ownerCode, bankAccount: bankNumbers[0] || '', bankName: String(bank.bankName || ''), bankHolder: String(bank.holderName || ''), bankIfsc: String(bank.ifsc || ''), upiId: upiIds[0] || '', isActive: true, balance: 399, depositBalance: 399, packageName: 'Free 399', packageAmount: 399, signupBonusAmount: 399, createdAt: new Date()});
      ...
      const bank = Array.isArray(payload.banks) ? payload.banks[0] || {} : {};
      fallback.users.push({username, phone, passwordHash: await hashPassword(password), userId, inviteCode, ownerCode, bankAccount: bankNumbers[0] || '', bankName: String(bank.bankName || ''), bankHolder: String(bank.holderName || ''), bankIfsc: String(bank.ifsc || ''), upiId: upiIds[0] || '', isActive: true, balance: 399, depositBalance: 399, packageName: 'Free 399', packageAmount: 399, signupBonusAmount: 399, walletLimit: 0, createdAt: new Date().toISOString()});
      fs.writeFileSync(fallbackDataPath, JSON.stringify(fallback, null, 2));
      return json(response, 201, {ok: true, userId, inviteCode, ownerCode});
