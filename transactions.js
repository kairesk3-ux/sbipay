(function(){
  ...
  const completePurchase=(orderId,utr)=>{if(!/^\d{12}$/.test(String(utr||'')))return null;const records=read();const record=records.find(item=>item.orderId===orderId&&item.direction==='purchase');if(!record||records.some(item=>item.utr===utr))return null;record.status='success';record.utr=utr;record.completedAt=new Date().toISOString();const commission=Number((record.amount*.05).toFixed(2));records.unshift({id:`commission-${orderId}`,code:orderId,type:'INR',currency:'INR',label:'Purchase Commission',direction:'receive',amount:commission,status:'success',date:new Date().toISOString(),sourceOrderId:orderId});write(records);const sales=JSON.parse(localStorage.getItem('sbiPaySellerSales')||'[]');sales.unshift({...record,status:'sold',sellerStatus:'Sold',sellerBalanceDeduction:record.amount});localStorage.setItem('sbiPaySellerSales',JSON.stringify(sales));const sellerRecords=JSON.parse(localStorage.getItem('sbiPaySellerTransactions')||'[]');sellerRecords.unshift({id:`seller-sale-${orderId}`,orderId,label:'Sale INR',direction:'sale',amount:record.amount,status:'success',utr,date:new Date().toISOString()});localStorage.setItem('sbiPaySellerTransactions',JSON.stringify(sellerRecords));const sellerBalance=Math.max(0,Number(localStorage.getItem('sbiPaySellerBalance')||0)-record.amount);localStorage.setItem('sbiPaySellerBalance',String(sellerBalance));const profile=JSON.parse(localStorage.getItem('sbiPayProfile')||'null');if(profile?.ownerCode){const referralCredits=JSON.parse(localStorage.getItem('sbiPayReferralCredits')||'[]');referralCredits.unshift({id:`referral-${orderId}`,ownerCode:profile.ownerCode,amount:commission,sourceOrderId:orderId,utr,status:'success',date:new Date().toISOString()});localStorage.setItem('sbiPayReferralCredits',JSON.stringify(referralCredits))}releaseOrder(orderId);return record};
